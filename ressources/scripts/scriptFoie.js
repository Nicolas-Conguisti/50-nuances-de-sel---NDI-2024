...
function loadPlantImages() {
  ...
  Object.keys(plantTypes).forEach((type) => {
    ...
    img.onerror = () => {
      ...
      if (altExt) {
        img.src = `./ressources/img/imagesFoie/${type}${altExt}`;
      } else {
        console.error(`Image not found for plant type: ${type}`);
      }
    };

    img.src = `./ressources/img/imagesFoie/${type}.png`; // Commence par essayer .png
  });
}
...
function loadZombieImages() {
  ...
  ['basic', 'fast', 'strong', 'boss'].forEach((type) => {
    ...
    img.onerror = () => {
      ...
      const altExt = extensions.find((ext) => ext !== img.src.slice(-4));
      if (altExt) {
        img.src = `./ressources/img/imagesFoie/zombies/${type}${altExt}`;
      } else {
        console.error(`Image not found for zombie type: ${type}`);
      }
    };

    img.src = `./ressources/img/imagesFoie/zombies/${type}.png`; // Commence par essayer .png
  });
}
